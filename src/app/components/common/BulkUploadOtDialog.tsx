...
import {
  parseOtExcel, downloadOtTemplate, toCreateOtRequest,
  ParsedOtData,
} from '../../utils/otBulkParser';
...
interface Props {
  ...
  onImported: () => void;
  /** Whole roster — the parser resolves Employee ID → backend UUID and
   *  reports name / dept in the preview using this list. */
  employees: Employee[];
}
...
/**
 * Bulk-file overtime rows from an Excel workbook. Mirrors the Vendors /
 * Customers bulk dialog. Gated at the caller — the Overtime page only
 * mounts this when the operator has full overtime CRUD AND tenant-wide
 * employee visibility (i.e. can legitimately file OT for anyone).
 */
export function BulkUploadOtDialog({
  open, onOpenChange, onImported, employees,
}: Props) {
  const [file, setFile] = useState<File | null>(null);
  const [parsing, setParsing] = useState(false);
  const [parsed, setParsed] = useState<ParsedOtData | null>(null);
  ...
  useEffect(() => {
    ...
    setSelectedRows(new Set(parsed.rows.filter(r => r.errors.length === 0).map(r => r.rowNumber)));
  }, [parsed]);
  ...
  const handleSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setProgress(new Map());
    try {
      const result = await parseOtExcel(f, employees);
      setParsed(result);
      const errorRows = result.rows.filter(r => r.errors.length > 0).length;
      if (result.errors.length > 0) {
        toast.error(result.errors[0]);
      } else if (errorRows > 0) {
        toast.error(`${errorRows} row${errorRows !== 1 ? 's' : ''} have issues — review before import.`);
      } else if (result.totalRows > 0) {
        toast.success(`Ready to import ${result.validRows} OT row${result.validRows !== 1 ? 's' : ''}`);
      } else {
        toast.error('No OT rows found in the workbook.');
      }
    } catch (err) {
  ...
  const handleImport = async () => {
    ...
    await runWithConcurrency(
      ...
      async (row) => {
        ...
        return overtimeApi.create(toCreateOtRequest(row));
      },
      5,
      (row, _i, result) => {
    ...
    if (okCount > 0) {
      ...
      toast.success(
        failCount === 0
          ? `Imported ${okCount} OT row${okCount !== 1 ? 's' : ''}`
          : `Imported ${okCount} of ${okCount + failCount} — ${failCount} failed`,
        { duration: 6000 },
      );
    }
    if (okCount === 0 && failCount > 0) {
      toast.error('No OT rows imported — every row failed. See the list for details.', { duration: 8000 });
    }
  };
  ...
  const summary = parsed ? {
    ...
    errorRows: parsed.rows.filter(r => r.errors.length > 0).length,
    warningRows: parsed.rows.filter(r => r.errors.length === 0 && r.warnings.length > 0).length,
  } : null;
            ...
            <TooltipProvider delayDuration={120}>
              <Tooltip>
                ...
                <TooltipContent side="right" className="max-w-xs text-xs leading-relaxed">
                  Upload an Excel file (.xlsx). One row per OT record. Rows resolve to employees by Employee ID (empNo); name is accepted as a fallback.
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </DialogTitle>
          ...
          {!parsed && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="p-4 rounded-md border-2 border-dashed border-gray-300 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-xs text-gray-500 mb-3">Overtime + Guide tabs with sample data</p>
                <Button variant="outline" size="sm" onClick={downloadOtTemplate}>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-amber-800">
                    Usually a repeated employee + date, or a name-matched row. Rows still import — untick them below if unwanted.
                  </p>
                </div>
              </div>
            </div>
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium">
                    {finalResult.failed === 0
                      ? `All ${finalResult.ok} OT row${finalResult.ok !== 1 ? 's' : ''} imported successfully`
                      : finalResult.ok === 0
                        ? `No OT rows imported — all ${finalResult.failed} failed`
              ...
              <table className="w-full text-xs">
                <thead className="sticky top-0 bg-gray-100 z-10">
                  <tr>
                    ...
                    <th className="text-center px-3 py-2 font-medium">Start</th>
                    <th className="text-center px-3 py-2 font-medium">End</th>
                ...
                <tbody>
                  {parsed.rows.map(r => {
                    ...
                    const empLabel = r.employee
                      ? `${r.employee.name}${r.employee.id ? ` · ${r.employee.id}` : ''}`
                      : r.data.employeeIdLabel;
                    return (
                      <tr key={r.rowNumber} className={`border-t ${rowBg}`}>
                        ...
                        <td className="px-3 py-2 font-medium truncate max-w-[200px]" title={empLabel}>{empLabel}</td>
                        <td className="px-3 py-2 text-gray-700 tabular-nums">{r.data.date}</td>
                        <td className="px-3 py-2 text-gray-600 tabular-nums">{r.data.endDate ?? ''}</td>
                        <td className="px-3 py-2 text-center text-gray-600 tabular-nums">{r.data.startHour ?? ''}</td>
                        <td className="px-3 py-2 text-center text-gray-600 tabular-nums">{r.data.endHour ?? ''}</td>
                        <td className="px-3 py-2 text-right text-gray-700 tabular-nums">{r.data.hours || ''}</td>
                        <td className="px-3 py-2 text-gray-600 capitalize">{r.data.dayType ?? ''}</td>
                        <td className="px-3 py-2 text-right text-gray-600 tabular-nums">
                          {r.data.rateOverride != null ? `${r.data.rateOverride}×` : ''}
                        </td>
              ...
              >
                {importing ? (
                  ...
                  <>
                    ...
                    {selectedRows.size === 0
                      ? 'No rows selected'
                      : `Import ${selectedRows.size} OT Row${selectedRows.size !== 1 ? 's' : ''}`}
                  </>
                )}
              </Button>
            )}
