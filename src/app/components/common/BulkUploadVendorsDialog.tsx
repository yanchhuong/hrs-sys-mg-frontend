...
import {
  parseVendorsExcel, downloadVendorTemplate, toVendorRequest,
  ParsedVendorData,
} from '../../utils/vendorBulkParser';
...
interface Props {
  ...
  existingVendors?: vendorsApi.Vendor[];
}
...
/**
 * Bulk-import vendors from an Excel workbook. Purchase-side twin of
 * {@code BulkUploadCustomersDialog}. Same UX skeleton; the parser
 * mirrors VendorService's stricter business-vendor rule (TIN AND
 * representative required — no {@code businessType} escape hatch).
 */
export function BulkUploadVendorsDialog({
  open, onOpenChange, onImported, existingVendors = [],
}: Props) {
  const [file, setFile] = useState<File | null>(null);
  const [parsing, setParsing] = useState(false);
  const [parsed, setParsed] = useState<ParsedVendorData | null>(null);
  ...
  useEffect(() => {
    ...
    setSelectedRows(new Set(parsed.vendors.filter(r => r.errors.length === 0).map(r => r.rowNumber)));
  }, [parsed]);
  ...
  const handleSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setProgress(new Map());
    try {
      const result = await parseVendorsExcel(f, existingVendors);
      setParsed(result);
      const errorRows = result.vendors.filter(r => r.errors.length > 0).length;
      if (result.errors.length > 0) {
        toast.error(result.errors[0]);
      } else if (errorRows > 0) {
        toast.error(`${errorRows} row${errorRows !== 1 ? 's' : ''} have issues — review before import.`);
      } else if (result.totalVendors > 0) {
        toast.success(`Ready to import ${result.validVendors} vendor${result.validVendors !== 1 ? 's' : ''}`);
      } else {
        toast.error('No vendor rows found in the workbook.');
      }
    } catch (err) {
  ...
  const handleImport = async () => {
    if (!parsed) return;
    const rowsToImport = parsed.vendors.filter(
    ...
    await runWithConcurrency(
      ...
      async (row) => {
        ...
        return vendorsApi.create(toVendorRequest(row));
      },
      5,
      (row, _i, result) => {
    ...
    if (okCount > 0) {
      ...
      toast.success(
        failCount === 0
          ? `Imported ${okCount} vendor${okCount !== 1 ? 's' : ''}`
          : `Imported ${okCount} of ${okCount + failCount} — ${failCount} failed`,
        { duration: 6000 },
      );
    }
    if (okCount === 0 && failCount > 0) {
      toast.error('No vendors imported — every row failed. See the list for details.', { duration: 8000 });
    }
  };

  const summary = parsed ? {
    total: parsed.totalVendors,
    valid: parsed.validVendors,
    errorRows: parsed.vendors.filter(r => r.errors.length > 0).length,
    warningRows: parsed.vendors.filter(r => r.errors.length === 0 && r.warnings.length > 0).length,
  } : null;
    ...
    <Dialog open={open} onOpenChange={(o) => {
      ...
      <DialogContent className="max-w-4xl flex flex-col max-h-[90vh] p-0 gap-0">
            ...
            <TooltipProvider delayDuration={120}>
              <Tooltip>
                ...
                <TooltipContent side="right" className="max-w-xs text-xs leading-relaxed">
                  Upload an Excel file (.xlsx). One row per vendor. Business rows require both TIN and Representative — individuals need neither.
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </DialogTitle>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-6 py-4 space-y-4">
          {!parsed && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="p-4 rounded-md border-2 border-dashed border-gray-300 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-xs text-gray-500 mb-3">Vendors + Guide tabs with sample data</p>
              ...
              <div className="p-4 rounded-md border-2 border-dashed border-gray-300 flex flex-col items-center justify-center text-center">
                ...
                <input
                  ...
                  id="bulk-vendors-file"
                  className="hidden"
                  disabled={importing}
                />
                <label htmlFor="bulk-vendors-file">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium">
                    {finalResult.failed === 0
                      ? `All ${finalResult.ok} vendor${finalResult.ok !== 1 ? 's' : ''} imported successfully`
                      : finalResult.ok === 0
                        ? `No vendors imported — all ${finalResult.failed} failed`
          ...
          {parsed && parsed.vendors.length > 0 && (
                  ...
                  {parsed.vendors.map(r => {
                    ...
                    return (
                      <tr key={r.rowNumber} className={`border-t ${rowBg}`}>
                        ...
                        <td className="px-3 py-2 text-gray-600 truncate max-w-[220px]">{r.data.address ?? ''}</td>
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {!finalResult && (
              <Button
                ...
                disabled={!parsed || parsed.totalVendors === 0 || selectedRows.size === 0 || importing}
              >
                {importing ? (
                  <>
                  ...
                  <>
                    ...
                    {selectedRows.size === 0
                      ? 'No vendors selected'
                      : `Import ${selectedRows.size} Vendor${selectedRows.size !== 1 ? 's' : ''}`}
                  </>
                )}
              </Button>
            )}
